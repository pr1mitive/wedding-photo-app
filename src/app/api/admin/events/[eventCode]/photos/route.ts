...
export async function GET(_: Request, { params }: { params: Promise<{ eventCode: string }> }) {
  ...
  const { data, error } = await supabaseAdmin
    .from('photos')
    .select('id, thumb_path, display_path, guest_name, comment, timeline_label, is_favorite, is_highlight, is_hidden, created_at')
  ...
  const photos = await Promise.all(
    (data || []).map(async (row) => {
      const [thumbSigned, displaySigned] = await Promise.all([
