...
export default function AdminSettingsForm({ eventCode }: Props) {
  ...
  return (
    <div className="wedding-card" style={{ padding: 24 }}>
      ...
      <div className="title-jp" style={{ fontSize: 24, marginTop: 8 }}>モニター・公開設定</div>
      <div style={{ marginTop: 12 }}><OrnamentDivider wide={68} /></div>

      <div style={{ marginTop: 22, display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0,1fr))', gap: 16 }}>
      ...
      <div style={{ marginTop: 18, display: 'grid', gap: 10 }}>
        ...
        <Toggle label="ハイライト画像を優先する" checked={form.highlightPriority} onChange={(checked) => setForm((s) => ({ ...s, highlightPriority: checked }))} />
      </div>

      <button type="button" className="btn-primary title-serif" style={{ marginTop: 22, width: '100%' }} onClick={save} disabled={saving}>
