...
export default function DisplaySlideshow({ eventCode }: Props) {
  ...
  const collageColumns = useMemo(() => {
    ...
    return Array.from({ length: COLUMN_COUNT }, (_, columnIndex) =>
      repeated.filter((_, idx) => idx % COLUMN_COUNT === columnIndex),
    );
  }, [photos]);

  const fetchPhotos = async () => {
    const res = await fetch(`/api/public/display/${eventCode}/photos?limit=120&orderType=chronological`, { cache: 'no-store' });
  ...
  useEffect(() => {
    fetchPhotos();
    const poll = setInterval(fetchPhotos, 6000);
    return () => clearInterval(poll);
  }, [eventCode]);

  useEffect(() => {
    if (!focusPhoto) return;
    const timer = setTimeout(() => setFocusPhoto(null), FOCUS_DURATION_MS);
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 2, padding: '28px 44px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div className="title-serif" style={{ color: 'var(--gold)', letterSpacing: '0.28em', fontSize: 12 }}>YUKI &amp; HARUTO</div>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', color: 'var(--ink-50)', marginTop: 4 }}>NEW POST FOCUS · 5 SEC</div>
        </div>
      </div>
      ...
      >
        {collageColumns.map((column, columnIndex) => {
          const duration = 28 + columnIndex * 5;
